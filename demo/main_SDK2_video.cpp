  ...
  */
  ...
  if (inputformat) {
    ...
    printf(
        ...
        "640 480 2 0. use max resolution if input is 0. Press ESC when video "
        "window is focused to quit capture\n");
                           ...
                           (ASI_IMG_TYPE)Image_type))  // IMG_RAW8
    {
      printf(
          ...
          "size(width*height) must be integer multiple of 1024\n");
  ...
  ASISetControlValue(CamInfo.CameraID, ASI_GAIN, 0, ASI_TRUE);
  ...
  while (bMain) {
    if (mode == ASI_MODE_NORMAL) {
      if (ASIGetVideoData(CamInfo.CameraID, (unsigned char*)pRgb->imageData,
